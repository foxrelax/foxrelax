...
class BERTEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # 将token映射成num_hiddens维的向量
        self.token_embedding = nn.Embedding(vocab_size, num_hiddens)
        # 将0或者1映射成num_hiddens维的向量
...
class MaskLM(nn.Module):
    ...
    # 参数:
    # X是BERTEncoder的输出, X.shape - (batch_size, num_steps, num_hiddens)
    ...
    def forward(self, X, pred_positions):
        ...
        # 那么`batch_idx`是`tensor([0, 0, 0, 1, 1, 1])`
        batch_idx = torch.repeat_interleave(
            batch_idx, num_pred_positions
        )  # batch_idx.shape(batch_size*num_pred_positions, )
        masked_X = X[
            batch_idx,
            pred_positions]  # masked_X.shape - (batch_size*num_pred_positions, num_hiddens)
